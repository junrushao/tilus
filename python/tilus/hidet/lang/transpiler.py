...
from tilus.hidet.lang.constructs.declare import Declaration
from tilus.hidet.lang.constructs.loops import HidetLoopIterable
from tilus.hidet.lang.constructs.meta import HidetMetaLoopIterable
from tilus.hidet.utils import blue, bold, red, str_indent
...
class PythonAstFunctor:
    ...
    def visit(self, node):
        ScheduleError = RuntimeError  # library.tune not available
...
class Scope:
    ...
    def default_top_level():
        scope = Scope(None)
        scope.define_host_var("range", tilus.hidet.lang.constructs.loops.range)
        return scope
...
class PythonToHidetTranslator(PythonAstFunctor):
    ...
    def process_assign(self, lhs: Union[Attribute, Subscript, Name], rhs, type_annotation: Optional[ast.expr] = None):
        ...
        if isinstance(lhs, Name):
            ...
            namespace = {
                tilus.hidet.lang.attrs: "",
                tilus.hidet.lang.attrs.cuda: "cuda.",
                tilus.hidet.lang.attrs.hip: "hip.",
            }
        ...
        with self.scope() as env_scope:
            ...
            with self.scope() as scope:
                ...
                for arg in args.args:
                    from tilus.hidet.lang.constructs.meta import HidetMetaParamTypeList
    ...
    def visit_Sequence(self, expr: Tuple | List) -> list:
        seq: list[Any] = []
        for v in expr.elts:
    ...
    def visit_UnaryOp(self, expr: UnaryOp):
        ...
        if isinstance(value, tilus.hidet.ir.Node):
    ...
    def visit_Call(self, expr: Call):
        ...
        if isinstance(func, types.FunctionType):
            ...
            # def f():
            #     ....
            from tilus.hidet.lang.script import ScriptModuleContext
