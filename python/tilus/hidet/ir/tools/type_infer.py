...
try:
    from tilus.hidet.ir.compute import ArgReduceCompute, GridCompute, ReduceCompute, ScalarInput, TensorInput
except ImportError:
    ArgReduceCompute = ReduceCompute = GridCompute = TensorInput = ScalarInput = None
...
try:
    from tilus.hidet.ir.cute.expr import CallOp
    from tilus.hidet.ir.cute.type import TiledTensorType, tiled_tensor
except ImportError:
    TiledTensorType = tiled_tensor = CallOp = None
...
class BinaryTypeInfer:
    def infer(self, lhs: BaseType, rhs: BaseType, op: Type[BinaryExpr]):
        if isinstance(lhs, DataType) and isinstance(rhs, DataType):
            ...
            Arithmetic = None  # cute not available
...
class ArithBinaryInfer(BinaryTypeInfer):
    def dtype(self, lhs: DataType, rhs: DataType, op):
        from tilus.hidet.ir.dtypes.promotion import promote_type
